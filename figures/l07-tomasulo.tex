...
\begin{tikzpicture}[x=1cm,y=1cm, font=\footnotesize\sffamily, >={Stealth[length=1.8mm]},
  ...
  \draw (0.55,-0.75) -- (9.3,-0.75);
  \fill (4.6,-0.75) circle (1.3pt);
  % ---- RAT and register file
  \node[blk, fill=ln-accent!15, minimum width=1.1cm] (rat) at (0.55,-1.45) {RAT};
  \node[blk, minimum width=1.1cm, font=\scriptsize\sffamily] (regs) at (0.55,-2.35) {\iflangja{レジスタ}{register\\file}};
  ...
  \node[blk, minimum width=1.6cm, minimum height=0.5cm] (aadd) at (9.3,-1.35) {\iflangja{アドレス加算器}{address adder}};
  \draw[->] (9.3,-0.75) -- (aadd.north);
  \node[anchor=south east, font=\scriptsize, text=ln-muted] at (9.3,-0.75) {LW, SW};
  \node[blk, minimum width=1.1cm, font=\scriptsize\sffamily] (sb) at (8.7,-2.3) {\iflangja{ストア\\バッファ}{store\\buffers}};
  \node[blk, minimum width=1.1cm, font=\scriptsize\sffamily] (lb) at (9.9,-2.3) {\iflangja{ロード\\バッファ}{load\\buffers}};
  \draw (aadd.south) -- (9.3,-1.8);
  \draw[->] (9.3,-1.8) -| (sb.north);
  \draw[->] (9.3,-1.8) -| (lb.north);
  % ---- execution units
  ...
  \node[blk, minimum width=2.3cm] (mem) at (9.3,-3.3) {\iflangja{メモリ}{memory}};
  ...
  \draw[bus] (0.55,-4.3) -- (10.6,-4.3);
  \node[anchor=north east, font=\scriptsize\bfseries\sffamily, text=ln-accent] at (10.6,-4.38)
    {CDB\enskip(\iflangja{結果書き込み}{write result})};
  \draw[->] (adder.south) -- (2.9,-4.3);
  \draw[->] (mult.south) -- (6.1,-4.3);
  \draw[->] (mem.south) -- (9.3,-4.3);
