...
\begin{tikzpicture}[x=1cm,y=1cm,>={Stealth[length=2mm]}, font=\small\sffamily]
  ...
  %% The (wider) Japanese label placed like the English one runs into the
  %% dashed total curve, so it goes left of the curve ends instead.
  \iflangja{\node[ln-accent,anchor=east] at (5.05,3.65) {動的電力 $\propto V^2$};}
    {\node[ln-accent,anchor=west] at (4.3,3.4) {active $\propto V^2$};}
