...
% \iflangja: the Japanese word for "disk" is too wide to repeat above every
% column, so the Japanese edition prints it once at the left of the row of
% column numbers.
\begin{tikzpicture}[x=1cm,y=1cm, font=\footnotesize\sffamily]
  ...
  \ja{\node[font=\scriptsize, text=ln-muted, anchor=east] at (-0.12,0.3) {ディスク};}
  \foreach \d in {0,1,2} {
    \node[font=\scriptsize, text=ln-muted] at ({\d*\blkgap+\blkw/2},0.3) {\iflangja{\d}{Disk \d}};
    \foreach \s in {0,...,3} {
