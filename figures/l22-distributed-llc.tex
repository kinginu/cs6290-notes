...
% slice 6.
% The Japanese labels are wider than the English ones, so the boxes inside
% a tile are a little wider in the Japanese edition; the tile outline, the
% routers and the links are the same in both.
\iflangja
  {\def\lnTileW{1.66cm}\def\lnTileHW{0.8cm}\def\lnTileL{-0.63}%
   \def\lnTileR{0.23}\def\lnTileSh{0.12cm}}
  {\def\lnTileW{1.5cm}\def\lnTileHW{0.72cm}\def\lnTileL{-0.59}%
   \def\lnTileR{0.19}\def\lnTileSh{0.2cm}}%
\begin{tikzpicture}[x=1cm,y=1cm, font=\scriptsize\sffamily, >={Stealth[length=1.8mm]},
  tile/.style={draw=ln-rule, rounded corners=2pt},
  core/.style={draw, fill=ln-accent!15, minimum width=\lnTileW, minimum height=0.34cm, inner sep=0pt},
  ...
  \foreach \i in {0,...,7} {
    ...
    \begin{scope}[shift={(\col*2.65,-\row*2.6)}]
      ...
      \node[box, minimum width=\lnTileHW] (l1\i) at (\lnTileL,0.18) {L1};
      \node[box, minimum width=\lnTileHW] (l2\i) at (\lnTileR,0.18) {L2};
      \node[box, minimum width=\lnTileW] (llc\i) at (-0.2,-0.26) {LLC~\i};
      \node[box, minimum width=\lnTileW] (dir\i) at (-0.2,-0.70) {\iflangja{ディレクトリ}{directory}~\i};
  ...
  \draw[->, hot, line width=0.9pt] (r6.west) -| ([xshift=\lnTileSh]llc6.east) -- (llc6.east);
\end{tikzpicture}
