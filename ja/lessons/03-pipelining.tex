...
\lessoninfo{
  ...
  keywords={パイプライン，ステージ，パイプライン CPI，ストール，フラッシュ，制御依存，データ依存，RAW，WAR，WAW，ハザード，フォワーディング，パイプライン深さ},
}
...
条件と飛び先は EX の終わりまで分からない．その時点で後続の 2 命令は
...
これが CPI に与えるコストは，どれだけ頻繁に起こるかに依存する．講義で
...
\begin{description}
  \item[RAW（read after write）] 後の命令が，先の命令が\emph{書く}値を
    \emph{読む}．\emph{真の依存}あるいは\emph{フロー依存}とも呼ばれる．
    データが実際に一方から他方へ流れなければならない．
  \item[WAR（write after read）] 後の命令が，先の命令が\emph{読む}位置に
    \emph{書く}．\emph{逆依存}．
  \item[WAW（write after write）] 両命令が同じ位置に\emph{書く}．後の
...
\begin{example}
  ...
  I3:  MUL R1, R6, R7     ; I1 と R1 について WAW，I2 と R1 について WAR
  \end{lstlisting}
\end{example}
...
\emph{時間を遡って}送ることはできないからである．ロードの結果は MEM の
...
\begin{example}
  ...
  ADD R3, R1, R4     ; EX の開始（サイクル 4）に R1 が必要: 1 サイクルストール
  \end{lstlisting}
