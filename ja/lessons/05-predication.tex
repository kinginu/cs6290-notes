...
られる．\caution{何も推測されず，取り消しも起こらない．両方向は意図的に
実行され，不要な結果はコード自身が捨てる．}分岐の実行 1 回当たり，プレディケーションは不要な方向の命令を無駄
...
% [tb]: [h] でここに置くと次の段落とその傍注がページ下端に押し出され，
% 傍注が本文領域の下にはみ出した．
\begin{table}[tb]
  \centering\small
...
の無駄は予測の無駄より少なくなる．\margin{実際のペナルティはこれより
大きい．Skylake 級のコアでは予測ミス 1 回が約 17 サイクル（第4章）であり，
1 サイクル当たり 4 命令なら約 70 命令分のスロットに当たる．}\cref{sec:l05-performance}では，
...
\margin{分岐の除去は一続きの直線的なコードを残すことにもなり，コンパイラは
その全体をまとめてスケジューリングできる（第10章）．}
...
\tip{\texttt{SLT} の後，条件レジスタは条件が成り立つときにだけ非ゼロで
ある．したがって \texttt{MOVN} が then~部を，\texttt{MOVZ} が else~部を
担う．}
...
\caution{$m$ はこの 1つの分岐の率，すなわち断片の実行 1 回当たりの予測ミス数
であり，第4章の命令当たりの率ではない．$P$ はサイクルではなく命令で数える．}
...
予測ミスペナルティを単位として測ったものである．\tip{変換後のコードの命令が
1つ増えるごとに，損益分岐率は $1/P$ 上がる．$P = 20$ なら 1 命令につき予測
精度 5 ポイントである．}\cref{sec:l05-predication}%
...
\margin{128 ビットのバンドルは 41 ビット命令 3つと 5 ビットのテンプレートを
収める．IA-64 には整数レジスタと浮動小数点レジスタも 128 本ずつある
（第11章）．}
...
\sidenote{IA-64 は 1994 年から HP と Intel が共同で開発したものであり，完全
プレディケーションは，並列性をハードウェアではなくコンパイル時に引き出すと
いう構想の一部であった．最初の Itanium は 2001 年に出荷されたが，x86-64 に
取って代わることはなく，Intel は 2021 年 7 月に最後の 1 台を出荷した
（第11章）．}
...
有利にする．\margin{ARM の 32 ビット命令 A32 は 4 ビットの条件フィールドを
持ち，ほぼすべての命令を条件付きで実行できる．AArch64（2011 年）はこれを
廃し，条件付きセレクト（CSEL）を残した．}
