...
するためである．そして深いパイプラインでは，予測ミスのたびに数十命令が
フラッシュされる．この章では，そのような分岐に対する代替手段である
\emph{プレディケーション}を調べる．プレディケーションは分岐の両方向の
処理を実行し，正しい方向の結果だけを残すので，予測ミスの対象となるものが
何も残らない．どの分岐に効果があるか，コンパイラが if~変換によって分岐を
どのように除去するか，それが前提とする条件付きムーブ命令，この変換が
有利になるのはいつか，そしてすべての命令を条件付きにできる命令セットを
順に調べる．
...
なる．したがって if~変換には，分岐せずに値を選択する命令が必要である．

\section{条件付きムーブ命令}
\label{sec:l05-cmov}

\term[じょうけんつきむーぶ]{条件付きムーブ}[conditional move]とは，条件が
成り立てば値を宛先にコピーし，成り立たなければ宛先を変更しない命令である．
\source{Lesson 5，動画 4--5；H\&P §H.4，§A.6．}この命令はどちらの場合も
実行され，書き込むかどうかだけが条件に依存する．制御の流れを変えないので，
予測すべきものは何もない．
...
$n_b$ 命令を実行し，実行 1 回当たり $m$ 回，ペナルティ $P$ 命令の予測ミスを
起こすとする．また変換版は $n_c$ 命令を実行するとする．if~変換が有利に
なるのは次の場合である．
\begin{equation}
  n_c \;<\; n_b + mP
...
\begin{definition}[完全プレディケーション]
  ...
  すべての命令に条件ビットが付加され，命令がその条件が成り立つときにだけ
...
選択された述語レジスタが~1 を保持するときにだけ効果を持ち，そうでなければ
無効化される．すなわち結果を書き込まず，例外も発生させない．そのため
条件付きムーブの場合と異なり，一方の方向に，もう一方の経路では安全でない
ロードやストアを含めてもよい．述語レジスタ~0 は常に~1 を保持するので，
限定述語が~0 の命令は無条件に実行される．述語レジスタは比較命令によって
設定される．比較命令は，条件に対するものとその否定に対するものという，
互いに相補的な値を持つ述語の対を書き込む．アセンブリ言語では，
\mbox{\texttt{(P1) ADD R1, R2, R3}} のように，限定述語を命令の前の括弧内に
書く．
...
\begin{figure}[htbp]
  ...
  \caption{IA-64 における完全プレディケーション．命令の限定述語（qp）が，
    64 個の 1 ビット述語レジスタのうち 1つを選択する．命令はそのレジスタが~1
    を保持するときにだけ実行され，そうでなければ無効化されて，書き込みも
    例外も起こらない．}
  \label{fig:l05-qp}
\end{figure}
...
\end{lstlisting}
  ここで \texttt{CMPLT} は，R1 が R2 より小さければ P1 を~1，P2 を~0 に
  設定し，そうでなければその逆に設定する比較命令を表す．加算そのものが
  条件付きなので，加算は変数に直接書き込み，比較命令が \texttt{SLT} と分岐
  の両方を置き換える．\cref{tab:l05-versions} は，\cref{ex:l05-performance}
  の数値を用いて 3つの版を比較したものである．
\end{example}
...
遅くなる（\cref{fig:l05-break-even}）．方向がより大きければ損益分岐率は
正になるが，条件付きムーブの場合よりもムーブの数を~$P$ で割った分だけ低い．
条件付きムーブが条件をすでに保持しているレジスタを検査し，比較を必要と
しない場合には，その差はムーブの数より 1 命令少ない数を~$P$ で割った分と
なる．損益分岐率を下げることで，完全プレディケーションは，条件付きムーブ
では引き合わないほど予測精度の高い分岐に対しても if~変換を有利にする．
