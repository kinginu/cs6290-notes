...
\begin{example}
  ...
  スループットが \SI{0.1}{programs/s} である．\margin{DDR4-3200 モジュールは
  読み出しコマンドから \SI{14}{\nano\second} でデータを返すが（CL22），
  アクセスが重なるため 25.6~GB/s を持続する．第15章．}プログラム当たり
...
からである．\caution{百分率は高速化率ではない．時間が \SI{30}{\percent}
短いことは $N \approx 1.43$ であって 1.3 ではない．}高速化率が 1 より
大きければ $X$ が優れ，1 未満なら劣る
...
\begin{description}
  ...
  \item[カーネル] 実アプリケーションの最も時間を消費する部分（例えば
    行列積）を切り出し，単独で実行できるようにしたもの．\margin{gem5 のような
    サイクルレベルシミュレータは，模擬する実機より3〜4桁遅い．}実行しやすく，
    実際の処理を反映している．
  \item[合成ベンチマーク] 実アプリケーションと\emph{同様に振る舞う}
    （命令やメモリアクセスの構成が似ている）ように書かれた，より実行の
    簡単なプログラム．設計の初期段階で有用．\sidenote{古典的な例は Whetstone
    （1972年，浮動小数点）と Dhrystone（Weicker，1984年，整数）である．
    いずれも，結果が使われないコードを最適化コンパイラが削除するようになって
    信頼を失った．合成プログラムがもはや最終的な測定として認められないのは
    このためである．}
...
プログラム，入力，報告規則について合意する．\margin{SPEC CPU2017 は43本の
プログラムを4つのスイート（整数と浮動小数点，それぞれ speed 版と rate 版）
に分けて含む．\texttt{spec.org}．}
...
\begin{itemize}
  ...
  \item \textbf{高速化率}（比）を算術平均しては\emph{ならない}．比の算術
    ...
    \begin{equation}
      ...
      \label{eq:geomean}
    \end{equation}
    \tip{$\exp\bigl(\frac{1}{n}\sum_i \ln S_i\bigr)$ として計算するとよい．
    多数の比の積はオーバーフローするが，対数の和はしない．}
\end{itemize}
...
\caution{幾何平均は一貫しているが予測的ではない．スイート全体の所要時間は
分からない．それを知るには時間を足し合わせる．}
...
\term[せいのうのてっそく]{性能の鉄則}[iron law]は，CPU
...
\begin{description}
  ...
  \item[クロックサイクル時間（$T_{\mathrm{clk}}$）] マイクロアーキテクチャ
    （2つのラッチの間にどれだけの論理があるか），\margin{Pentium~4（Prescott）
    は31段で \SI{3.8}{\giga\hertz}，2006年の Core は14段で
    \SI{3}{\giga\hertz} 未満だが，より速かった．}回路設計，製造技術で
    決まる．
\end{description}
...
命令クラスごとに必要なサイクル数が異なる場合，CPI は重み付き平均に
なる．\caution{ベンダは IPC $= 1/\text{CPI}$ で表すことが多い．スイート全体で
IPC を平均することには，高速化率を平均するのと同じ欠陥がある．}$n$ 個の
クラスについて，
\begin{equation}
...
\begin{example}
  ...
  分岐（3 サイクル）である．\margin{現実的な桁としては，SPEC の整数プログラム
  はロードが約 \SI{25}{\percent}，ストアが \SI{10}{\percent}，分岐が
  \SI{15}{\percent} である．H\&P 付録 A．}平均 CPI は
...
\begin{example}
  ...
  速くしても $1/0.6 \approx 1.67$ にしか届かない．\sidenote{Amdahl の3ページの
  論文（AFIPS 1967）は，この上限を大規模並列マシンへの反論に用いた．時間の
  \SI{95}{\percent} が並列であっても，コア数によらず $20\times$ を超えない．
  第18章でコアに適用する．}
\end{example}
...
講義では冗談めかして「Lhadma の法則」\caution{「Lhadma」は「Amdahl」を逆から
綴ったもので，講義独自の呼び名であり標準的な用語ではない．}と呼ばれる逆命題も
述べられる．頻出
...
\begin{example}
  ...
  高速化で $S = 1/(0.6+0.2) = 1.25$ が得られる．\margin{Amdahl の法則は問題の
  規模が固定であることも仮定している．マシンとともに問題が大きくなれば，
  直列部分の割合は小さくなる（Gustafson 1988）．}新しいマシンでは FP は
...
\begin{marginfigure}
  \resizebox{\linewidth}{!}{\input{figures/l02-amdahl}}
  \caption{$f_{\mathrm{enh}} = 0.4$ のときの全体の高速化率 $S$．改善部分が
    速くなるほど，同じ部分をさらに改善して得られる効果は小さくなる．}
