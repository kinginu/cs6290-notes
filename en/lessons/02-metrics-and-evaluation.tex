...
\begin{example}
  ...
  \SI{0.1}{programs/s}.  A second processor that takes \SI{15}{\second} per
...
speed.  A speedup greater than 1 means $X$ is better; a speedup below 1
means it is worse (a \enquote{speedup of 0.8} is a slowdown).  Speedups
compose multiplicatively: if $X$ is $2\times$ faster than $Y$ and $Y$ is
$3\times$ faster than $Z$, then $X$ is $6\times$ faster than $Z$.
...
usually unknown---so we resort to \term{benchmarks}[ベンチマーク]: programs,
...
\begin{description}
  ...
  \item[Kernels] the most time-consuming part of a real application (e.g.\ a
    matrix multiply), extracted so that it can be run on its own.  Easier to
    run; still reflects the real work.
  \item[Synthetic benchmarks] programs written to \emph{behave like} real
    ...
    being simpler to run.  Useful early in a design.
...
these organizations agree on the programs, inputs and reporting rules.
...
\begin{description}
  ...
  \item[Clock cycle time ($T_{\mathrm{clk}}$)] determined by the
    microarchitecture (how much logic sits between two latches), the circuit
    design and the fabrication technology.
\end{description}
...
CPI is a weighted average.  With $n$ classes,
\begin{equation}
...
\begin{example}
  ...
  (3 cycles).  The average CPI is $0.6\cdot 1 + 0.3\cdot 2 + 0.1\cdot 3 =
  1.5$, so the CPU time is $10^{9}\times 1.5 \times \SI{0.5}{\nano\second}
  = \SI{0.75}{\second}$.
\end{example}

\section{Amdahl's law}

\term{Amdahl's law}[Amdahl1967@Amdahl's law] predicts the overall speedup
...
\begin{example}
  ...
  fast would only reach $1/0.6 \approx 1.67$.
\end{example}
...
law}: \emph{do not sacrifice the uncommon case too much} when speeding up the
...
\begin{example}
  ...
  the first $2\times$ FP speedup gives $S = 1/(0.6+0.2) = 1.25$.  A second
