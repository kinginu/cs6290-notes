...
\term{Computer architecture}[コンピュータアーキテクチャ] is the discipline
of designing a computer that is well suited to its purpose, using the
fabrication technology that is available.\source{Lesson 1, videos 2--3;
H\&P §1.1--1.3.}  Two points in this definition deserve emphasis.
...
\begin{itemize}
  ...
  \item \emph{Available technology.}  The architect does not design
    ...
    dissipate---because those numbers bound what is achievable.
\end{itemize}
...
following consequences, all with the same 18--24-month doubling period:
\begin{itemize}
...
\begin{description}
  ...
  \item[Power] how much energy it consumes, which sets battery life for
    portable devices and cooling cost for servers;
...
\term{Dynamic power}[動的電力] is consumed by charging and discharging the
capacitance of wires and gates whenever a signal switches:
\begin{equation}
...
and on how much logic is left idle.}
...
\begin{itemize}
  ...
  \item Doubling the frequency at constant voltage doubles the power; the
    ...
    hardware) costs power only through the larger $C$.
\end{itemize}
...
\term{Static power}[静的電力] (leakage power) is consumed even when no
signal switches, because transistors are imperfect switches and leak current
...
\begin{marginfigure}
  \resizebox{\linewidth}{!}{\input{figures/power-vs-voltage}}
...
Chips are manufactured on circular silicon \term{wafers}[ウェハ], each of
which is cut into many identical rectangular \term{dies}[ダイ].  The cost of
...
raises the yield.  Read together with Moore's law this offers the architect
two options each technology generation:
\begin{itemize}
  \item keep the die area constant and pack in twice the transistors---a
    more capable chip at roughly the same cost; or
