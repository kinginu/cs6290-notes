...
never carry out within a clock cycle.  \Cref{tab:l10-overview} lists the
...
processor of Lesson 6 each level of the tree takes one cycle.
...
\end{lstlisting}
  ...
  is two further registers, R11 and R12, for intermediate results.
\end{example}

\begin{figure}[htbp]
  \centering
  \input{figures/l10-tree-height}
...
compilers regroup it only when permitted.}
...
instructions only up to the first one that must wait.  Three compiler
...
\begin{description}
  ...
  \item[Address offset] When an instruction that adds a constant to a
    ...
    \texttt{ADDI R1, R1, 4} becomes \texttt{SW R3, -4(R1)}.
\end{description}
...
\end{lstlisting}
  ...
  the last element, it compiles into
  \begin{lstlisting}[language={[x86masm]Assembler}, numbers=none]
...
\end{lstlisting}
  ...
  the iteration.  The LW and the ADDI now execute together, the ADD follows,
...
Scheduling thus matters for in-order processors, for out-of-order processors
...
even when the instructions before and after it are independent.
...
the gain.  Two further problems concern the number of iterations.  When $n$ is
not a multiple of $k$, the remaining iterations need additional code.  And
...
and often the saving and restoring of registers on the stack.
...
processor no longer has to predict the call and the return (Lesson 4).
...
cancel the gain.  Compilers therefore inline small functions, whose call
...
parallel steps entirely to the compiler: the VLIW processors of Lesson 11.
