...
\begin{description}
  ...
  \item[True data dependences] RAW dependences cannot be removed: an
    instruction has to wait until the values it reads have been computed.
...
Current processors schedule instructions in essentially the same way.  The
differences are listed in \cref{tab:l07-modern}.  The first two concern only
the scope of the algorithm; the third changes its mechanism.  Tomasulo's
algorithm has no provision for exceptions: once instructions have executed
out of order, the registers need not correspond to any point in the program
at which an exception could be taken.  Supporting exceptions requires an
additional structure, the subject of Lesson~8.
...
multiplications and divisions, served by one multiplier.  The name of a
...
operands are all values holds a \emph{ready} instruction.  When the execution
...
affects only performance and hardware cost.
\begin{description}
...
known only once both addresses have been computed.

Tomasulo's algorithm therefore performs loads and stores in program order.
They are issued like other instructions, with their operands resolved through
...
whose value has not been computed yet captures it from the CDB.  The memory
accesses are made from these buffers in the order in which the loads and
stores were issued.  A load broadcasts the value it reads on the CDB under its
own tag, exactly like an arithmetic result.  Modern processors also execute
loads and stores out of order, which requires the processor to detect and
handle the memory dependences; Lesson~9 covers this.
...
\begin{itemize}
  ...
  \item The adder executes ADD and SUB in 2 cycles; the multiplier executes
    MUL and DIV in 4 cycles.  An instruction dispatched in cycle $D$ with an
    execution time of $k$ cycles executes in cycles $D$ to $D+k-1$ and
    broadcasts in cycle $D+k$, in which its unit can already accept the next
    instruction.
...
file holds R1 $=10$, R2 $=2$, R3 $=620$, R4 $=62$, R5 $=618$ and R6 $=8$,
...
earlier, so the rows cannot simply be completed in program order.  Units and
...
\begin{example}
  ...
  agrees with the full trace of \cref{tab:l07-trace}.
\end{example}
