...
Lessons 18--21 developed shared-memory multiprocessors with a few cores: the
cores share a bus and a last-level cache, keep their caches coherent by
snooping on the bus, coordinate with locks, and rely on a consistency model
for shared data.  Every technology generation fits more cores on a chip,
however, and mechanisms that serve four or eight cores well break down when
a chip has dozens or hundreds of them.  This lesson examines the challenges
...
\begin{description}
  \item[Scaling] Adding cores enlarges the grid, and every added core adds
    links: as the traffic grows with the number of cores, so does the
    capacity of the network.  Every link connects two neighbouring cores and
...
\begin{figure}[htbp]
  ...
  \caption{Interconnects for many cores.  A bus carries one transfer at a
    ...
    different links; the torus adds wraparound links to every row and
    column.}
...
\emph{flattened butterfly} and the \emph{hypercube} link each router to many
more routers, which shortens the paths further but requires many long,
crossing links that are hard to build on a chip.

If the traffic is spread evenly over the links, a network of $L$ links, each
carrying up to $C$ messages per second, carries at best
\begin{equation}
...
$L=2k(k-1)$ links.  For source and destination tiles chosen independently
at random, the average distance is $2(k^{2}-1)/(3k)\approx 2k/3$ hops, which grows only with the square root
of the number of cores $N=k^{2}$, so the capacity of the mesh keeps growing
with $N$, whereas a bus never carries more than one link's worth of
traffic.\caution{A mesh raises throughput, not latency: a message to a
distant tile passes many routers, each adding delay.}

\begin{example}
  \label{ex:l22-throughput}
  Let each link, and the bus, carry $C$ messages per second.  With 16 cores
  the bus carries $C$; a $4\times4$ mesh has 24 links, random traffic
  travels 2.5 hops on average, and by \cref{eq:l22-throughput} the mesh
  carries up to $24C/2.5\approx 9.6C$.  With 64 cores the bus still carries
  at most $C$ (in practice less, since it is longer), while an $8\times8$
  mesh has 112 links and an average distance of 5.25 hops, for about $21C$.
  If all traffic went to neighbouring tiles ($h=1$), the two meshes would
  carry $24C$ and $112C$.
\end{example}
...
\begin{figure}[htbp]
  \centering
  \input{figures/l22-distributed-llc}
...
\begin{example}
  \label{ex:l22-llc}
  A chip has 16 tiles with a \SI{2}{MB} LLC slice each, so its LLC holds
  ...
  $2^{12}=4096$.  If the tile is selected by cache index, the 16-bit set
  number consists of address bits 6--21: its lowest 4 bits (address bits
  6--9) select one of the 16 tiles, the remaining 12 bits (address bits
  10--21) select the set within that slice, and the tag consists of the
...
Two decisions make the directory small enough for the chip.  The first is
where an entry is kept.  The directory is divided among the tiles, and the
entry of a block resides in the tile that holds the block's LLC slice, its
\emph{home} tile.  A core whose private caches miss must contact that tile
anyway to search the LLC, so it reaches the directory entry without an
additional message.  The second is which blocks have an entry at all.
...
\begin{marginfigure}
  \resizebox{\linewidth}{!}{\input{figures/l22-power-perf}}
...
\begin{equation}
  ...
  \label{eq:l22-perf}
\end{equation}
relative to the single core.  Beyond some number of cores the loss in
frequency outweighs the gain in parallelism (\cref{fig:l22-power-perf}).
...
processors of 2013 that the lecture uses as examples.

\begin{table}[htbp]
  \centering\small
  \caption{Normal and turbo clock frequencies of two quad-core Intel Core
    processors.  The power factor is the cube of the frequency factor.}
  \label{tab:l22-turbo}
...
turbo mode.  Dividing each design power evenly among the four cores (a rough
approximation), a turbo core dissipates about \SI{28}{W} in the laptop and
\SI{29}{W} in the desktop processor.
...
\begin{keypoint}[Lesson summary]
  ...
  network on chip, usually a mesh of tiles whose capacity grows with the
  number of cores (\cref{eq:l22-throughput}), and use directory-based
  coherence.  A distributed LLC gives each tile a slice, selected by cache
  index or, for locality, by page number; the coherence directory becomes an
  on-chip partial directory in the tile of each block's LLC slice.  A shared
  power budget lowers the frequency of every core by $\sqrt[3]{N}$
  (\cref{eq:l22-freq}), which turbo mode partly recovers for sequential
  programs, and the operating system must know which logical processors
  share a core or a chip.
\end{keypoint}
