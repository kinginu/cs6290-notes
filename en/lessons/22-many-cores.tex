...
\lessoninfo{
  ...
  textbook={H\&P \cite{hennessy2017} §1.5, §5.4, Appendix F},
...
number of cores.\margin{The lecture's \enquote{hundreds} has arrived: AMD's
EPYC 9965 puts 192 cores in one socket within a \SI{500}{W} budget, about
\SI{2.6}{W} per core.}  On a shared bus this traffic soon becomes the
bottleneck:
\begin{itemize}
  ...
  \item a bus that connects more cores is longer and has more devices
    attached, so each transfer is slower as well.\margin{Intel's last
    front-side bus, shared by every socket, peaked at 12.8~GB/s; QuickPath
    replaced it with point-to-point links in 2008.}
\end{itemize}
...
structures that all cores share.\sidenote{Intel's server chips made the
same move: the Xeon Scalable family of 2017 replaced the ring of earlier Xeons by a mesh, to
escape the \enquote{latencies and bandwidth constraints associated with
previous ring-based architecture} (Intel, \emph{Xeon Processor Scalable
Family Technical Overview}), and every server Xeon since has used one.
Tilera's TILE64 had already put 64 tiles of processor, cache and router on a
mesh in 2007.}
...
requires long, crossing links that are hard to build on a chip.\margin{Off
the chip they are practical: IBM's Blue Gene/L used a 3-D torus and Blue
Gene/Q a 5-D one, and Cray's XC supercomputers a dragonfly network.}
...
square root of the number of cores $N=k^{2}$.\tip{Along one dimension of a
$k$-node line the mean distance is $(k^{2}-1)/3k$; two dimensions, with the
source tile itself excluded, give exactly $2k/3$.}
...
small and fast and the requests of the cores are spread over many
slices.\margin{Intel's server LLC is sliced exactly so: \SI{1.375}{MB} per
core on the Xeon Scalable family, \SI{38.5}{MB} over the 28 cores of a
Platinum 8180.}
...
\begin{description}
  \item[By cache index] The lowest bits of the set number select the tile, so
    ...
    distant tile.\caution{Real chips do not take a field of the address:
    Intel selects the slice with an undocumented hash that XORs many
    physical address bits, so that even strided accesses spread over the
    slices \cite{irazoqui2015}.}
...
set.\sidenote{Intel's Xeon Scalable family needed exactly this structure.
Its earlier LLC was inclusive, so the LLC tags themselves recorded which
private caches held a block; the 2017 redesign made the LLC non-inclusive
(\SI{1.375}{MB} per core against \SI{1}{MB} of L2) and added a separate
\emph{snoop filter} to track \enquote{cache lines in the L1 or MLC of cores
when it is not allocated in the LLC} (Intel, \emph{Xeon Processor Scalable
Family Technical Overview}).}  A block that is only in the LLC or in main
memory would have an entry of zeros, and a missing entry says the same thing.  The directory therefore
...
\begin{equation}
  ...
  \label{eq:l22-perf}
\end{equation}
relative to the single core.\tip{\Cref{eq:l22-perf} is largest at
$N=2f_{\mathrm{par}}/(1-f_{\mathrm{par}})$: 18 cores for
$f_{\mathrm{par}}=0.9$, 198 for $0.99$.}  More generally, if a fraction
$t_{p}$ of the single-core execution time can use $p$ threads, the program needs
...
turbo mode.\margin{A current part shows the same gap: the Core i9-14900K
runs its performance cores at \SI{3.2}{GHz} within \SI{125}{W} and reaches
\SI{6}{GHz} within a \SI{253}{W} turbo power.  Intel ARK.}
...
threads per core, for example, appear as 32 logical
processors.\caution{\texttt{nproc} and \texttt{hardware\_concurrency} count
logical processors, not cores: on a 2-way SMT machine they return twice the
number of cores, and that many compute-bound threads run two per core.}
...
(\cref{fig:l22-placement}b).\margin{Linux publishes the topology in sysfs
(\texttt{core\_cpus} for the SMT siblings, \texttt{package\_cpus} for the
chip) and builds its scheduling domains from it; \texttt{lscpu} and
\texttt{lstopo} print it.}
