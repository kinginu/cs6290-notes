...
Compared with a direct-mapped cache of the same size,\margin{Why 32~KB 8-way L1 caches are common: 64
sets of 64-byte blocks span 4~KB, so index and offset fit in the page offset
and the lookup can start before translation (Lesson 14).} an $N$-way
...
the access time of main memory, which is what the cache was meant to avoid.\caution{The two choices are independent: write-through or write-back
...
% [tb], not p: with a float page the lesson ended on a page holding one line
% and the summary box; Table 12.4 is placed after the closing paragraph for
% the same reason.
\begin{figure}[tb]
  \centering
