...
cache cannot hold everything, and misses are therefore unavoidable.

\begin{figure}[htbp]
  \centering
  \input{figures/l12-cache-position}
...
\begin{example}
  \label{ex:l12-amat}
  Main memory adds a miss penalty of \SI{60}{\nano\second}.  Three candidate
  caches are compared:
  \begin{center}\small
    \begin{tabular}{@{}lccc@{}}
      \toprule
...
that a hit takes only one to three clock cycles.
...
\end{itemize}
Typical block sizes are 32--128 bytes.  The number of blocks a
...
Block sizes, and therefore line sizes, are always powers of two.  Only then
...
(\cref{sec:l12-write}): it must be written back first.

A lookup is a hit only if the line is valid and its tag matches:
\begin{equation}
  \text{hit} \;=\; V \wedge (\text{tag} = \text{block number}) .
  \label{eq:l12-hit}
\end{equation}
...
\begin{keypoint}
  ...
  offset picks the byte.  Everything else in cache organization decides
  \emph{which} lines have to be searched.
\end{keypoint}
...
\begin{description}
  ...
  \item[Fully associative] each block can be placed in any line of the
    cache (\cref{sec:l12-fa}).
\end{description}

\begin{figure}[htbp]
  \centering
  \input{figures/l12-placement}
...
\begin{figure}[htbp]
  \centering
  \input{figures/l12-direct-mapped}
...
cache is empty.  These conflicts raise the miss rate.
...
\begin{figure}[htbp]
  \centering
  \input{figures/l12-set-associative}
...
associative cache slow and expensive.
...
\begin{keypoint}
  ...
  more tag comparisons on every access but fewer blocks competing for the
  same lines.
\end{keypoint}
...
occurs exactly once in each set.  The line with counter~0 is the least
...
cache is also write-allocate; otherwise every write to a block that is not in
the cache would go to main memory, however often the block is written.
...
\begin{example}
  ...
  writes in total; write-through would need one per write, i.e.~4.
\end{example}
...
\begin{example}
  ...
  five accesses; both lines of the set are valid throughout.  The first
...
\begin{example}
  ...
  about \SI{2.3}{\percent} of the 64~KB of data.
\end{example}
