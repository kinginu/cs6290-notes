...
exception handler, usually part of the operating system.\caution{H\&P reserves
\emph{interrupt} for an event that no instruction causes, such as a timer tick
or an I/O completion; the mechanism of this chapter serves both.}  When the
handler
...
instruction that raised it does not exist.\caution{\emph{Phantom
exception} is this course's term; H\&P speaks of an exception raised by a
speculated instruction, buffered and discarded with it.}
...
allocated in no particular order.\margin{Smith and Pleszkun
\cite{smith1988} give three more ways to the same end: a result shift register,
a history buffer and a future file.}
...
instruction can be issued until the oldest one leaves the ROB.\margin{ROBs have
grown from 40 entries on the Pentium Pro to 224 on Skylake (Lesson~7), 256 on
AMD Zen~3 and 512 on Intel Golden Cove.}  Every issued
...
\emph{retirement}.\margin{\emph{Retire} is the word of the vendor
manuals: Intel's counter \texttt{INST\_RETIRED.ANY} counts commits, and measured
IPC (Lesson~2) is derived from it.}
...
ROB entries, and the register file is written only by commit.\sidenote{Keeping
results in the ROB, as here, is the design of Intel's P6 cores (Pentium Pro,
1995).  Since Sandy Bridge (2011) every result is written straight into the
physical register file of Lesson~6, and the ROB entry records only which
physical register the instruction was given; commit then changes a mapping
instead of copying a value, and recovery frees the registers allocated after
the branch.}  An instruction
passes through six steps: fetch, decode, issue, execute (from dispatch until
the operation completes), write result and commit.

\begin{figure}[htbp]
  \centering
  \input{figures/l08-organization}
...
before the branch are unaffected and continue to execute and commit.\margin{The
branch can be held up for a long time: a 32-bit integer division alone takes 26
cycles on Skylake (Agner Fog, \emph{Instruction tables}).}
...
\begin{description}
  \item[Precise exceptions] When the excepting instruction reaches the commit
    ...
    return, the later instructions are executed from that state.\margin{Demand
    paging rests on this: the handler fetches the missing page, and the
    faulting load is then executed again (Lesson~13).}
...
writer has been issued in the meantime, and only then is it reset.\tip{Only the
youngest writer of a register ever resets its RAT entry; an older one writes
the register file and leaves the RAT alone.}  The
...
\begin{itemize}
  ...
  \item an instruction commits no earlier than the cycle after its write
    result, and at most one instruction commits per cycle.\margin{One commit
    per cycle is an assumption of this example: Skylake retires up to four
    micro-operations per cycle and AMD Zen~3 up to eight.}
\end{itemize}
...
One instruction is issued per cycle, I$k$ into ROB$k$.\tip{Follow one register
at a time: its RAT entry changes when a writer issues, and a reader takes
whatever the RAT names in its own issue cycle.}  I1 copies 48 and 4
...
register file is unchanged.\tip{Entries in use $=$ (issue $-$
commit) $\bmod\ 8$: at the end of cycle~6 that is $(7-1) \bmod 8 = 6$, namely
ROB1 to ROB6.}
...
I4, which precede it.  The Commit column is increasing.\margin{A ROB is sized to cover the
longest stall it should hide: at four instructions per cycle, 224 entries hold
some 56 cycles---an L3 hit on Skylake takes 42 cycles, a DRAM access about 200
(7-cpu.com).}
I2 and I5 commit six
...
used more effectively.\margin{Intel's schedulers are
unified (97 entries on Skylake, Lesson~7); AMD's Zen cores keep separate
integer and floating-point schedulers.}  The cost lies in dispatch: instead of
choosing among
...
\begin{description}
  ...
  \item[Commit] is in program order again; the ROB restores the order that
    execution relaxed.\caution{The ROB reorders nothing: entries enter and
    leave it in program order.  The name refers to its effect on execution,
    not to the queue itself.}
\end{description}

\begin{figure}[htbp]
  \centering
  \input{figures/l08-order}
