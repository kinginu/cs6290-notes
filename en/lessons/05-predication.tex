...
away.  Per execution of the branch, predication wastes the instructions of
the unneeded direction, while prediction wastes the misprediction penalty
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.17\linewidth}>{\raggedright\arraybackslash}p{0.27\linewidth}>{\raggedright\arraybackslash}p{0.27\linewidth}>{\raggedright\arraybackslash}p{0.19\linewidth}@{}}
    ...
    Call, return       & always taken; BTB and RAS give the target
                       & no second direction to execute
                       & prediction \\
    \addlinespace
...
than one execution in ten is mispredicted.  \Cref{sec:l05-performance} makes
this comparison precise for the code that a compiler actually produces.
...
\begin{figure}[htbp]
  \centering
  \input{figures/l05-if-conversion}
...
without a branch.  Translated into ordinary branch instructions, each
...
addition execute the work of both directions.  If-conversion therefore needs
instructions that select a value without branching.
...
unchanged otherwise.\source{Lesson 5, videos 4--5; H\&P §H.4, §A.6.}  The
instruction is executed in either case; only whether it writes depends on the
condition.  It does not change the flow of control, so there is nothing to
predict.
...
\texttt{SLT}, which writes 1 if the condition holds and 0 otherwise.
...
\begin{example}
  ...
  \begin{lstlisting}[language={[x86masm]Assembler}, numbers=none]
      SLT  R3, R1, R2     ; R3 <- (v < t)
      BEQ  R3, R0, Else   ; v >= t: go to else part
      ADD  R4, R4, R1     ; lo <- lo + v
      ADDI R5, R5, 1      ; n  <- n + 1
      J    Done           ; skip else part
Else: ADD  R6, R6, R1     ; hi <- hi + v
Done: ...
\end{lstlisting}
...
let the converted version execute $n_c$ instructions.  If-conversion pays off
when
\begin{equation}
  n_c \;<\; n_b + mP
...
penalty.  Compared with the rough comparison of \cref{sec:l05-predication},
the extra instructions include the conditional moves, less the branch and
jump instructions that the conversion removes.
...
\term{predicate register}[述語レジスタ].  The instruction takes effect only
if the selected predicate register holds~1; otherwise it is annulled: it
neither writes a result nor raises an exception, so, unlike with conditional
moves, a direction may contain loads and stores that would be unsafe on the
other path.  Predicate register~0 always holds~1, so an instruction whose
qualifying predicate is~0 is unconditional.
...
its negation.  In assembly language the qualifying predicate is written in
parentheses before the instruction, as in \mbox{\texttt{(P1) ADD R1, R2,
R3}}.

\begin{figure}[htbp]
  \centering
  \input{figures/l05-qp}
  \caption{Full predication in IA-64.  The qualifying predicate (qp) of an
    ...
    instruction is executed only if that register holds~1; otherwise it is
...
\end{lstlisting}
  where \texttt{CMPLT} denotes a compare instruction that sets P1 to~1 and P2
  to~0 if R1 is less than R2, and the reverse otherwise.  The additions
...
both directions, and nothing else.  Compared with the branch version it
...
misprediction makes it slower (\cref{fig:l05-break-even}).  With larger
directions the break-even rate is positive, but lower than with conditional
moves by the number of moves divided by~$P$, or by one instruction less if
the conditional moves test a register that already holds the condition and so
need no compare.
By lowering the break-even rate, full predication makes if-conversion pay off
also for branches that are predicted too well to justify conditional moves.
