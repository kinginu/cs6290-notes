...
instruction of up to $N$ operations that the compiler has made
independent.\caution{A wrong grouping is not slow but wrong: nothing checks,
so two dependent operations in one instruction are executed anyway.
Correctness rests on the compiler.}

\begin{figure}[htbp]
  \centering
  \input{figures/l11-windows}
...
\begin{definition}[Very long instruction word]
  ...
  executed in parallel in the same cycle.\margin{Real width: an IA-64
  \emph{bundle} is 128~bits---three 41-bit operations and a 5-bit template
  naming their execution units (Intel Itanium architecture manual).}  Choosing
  the operations that form an instruction, and ensuring that they are
  independent, is the task of the compiler; the processor performs no
  dependence checking.
\end{definition}
...
\end{lstlisting}
  ...
  (\cref{tab:l11-packing}).\tip{Lower bound: $\lceil n/N\rceil$ instructions,
  and never fewer than the longest dependence chain has
  operations---$\max(2,4) = 4$ here.  Packing in program order can cost one
  more, as the two-slot column shows.}  With four slots the eight operations
  occupy four instructions, hence four cycles, where an ordinary ISA needs
  eight instructions; with two slots they occupy five.
\end{example}
...
\begin{itemize}
  \item \textbf{Latencies are not known at compile time.}  The compiler
    ...
    keeps executing instructions that do not depend on it.\margin{The number
    the compiler must assume: on Skylake a load takes 4 cycles from L1, 12
    from L2, about 40 from L3 and 200--300 from DRAM (Intel optimization
    manual).}
  \item \textbf{Irregular code is hard.}\tip{The first two are what the
    compiler cannot know: the actual latencies, the actual addresses and the
    actual branch directions.}  The compiler sees the program, not
...
was compiled for.\sidenote{Software can supply the missing compatibility.
Transmeta's Crusoe (2000) executed no x86 instruction directly: a translator,
the code morphing software, compiled blocks of x86 code into 128-bit VLIW
instructions of four operations and cached them \cite{dehnert2003}; the later
Efficeon used 256-bit instructions.  Nvidia's Denver (2014) translates Arm
code the same way.}
...
provides many.\margin{IA-64 gives the compiler 128 integer and 128
floating-point registers to rename into, against 32 in MIPS and 16 in x86-64
(Lesson 5).}
...
parallel instruction computing \cite{schlansker2000}.\margin{IA-64 uses stops
in the same way to end groups of independent instructions
(\cref{sec:l11-instructions}).}  The instruction set, however, accumulated
many additional features whose hardware became very complex, removing the main
advantage of VLIW.  The compiler still found little parallelism in irregular
code, and Itanium did not displace out-of-order processors.
...
for digital signals such as audio, video and radio.  This market plays to the
strengths of VLIW (\cref{sec:l11-good-bad}).\margin{H\&P lists the TI C6x
DSPs as an example of the VLIW approach.}\caution{The C6x marks the complement
of a stop bit: the lowest bit of an operation set to~1 means \emph{in parallel
with the next one}, so a group ends where that bit is~0; eight 32-bit
operations make a 256-bit packet (TI C6000 reference guide).}
\begin{itemize}
  ...
  \item Programs are usually compiled for the particular DSP of the device
    they run in, so backward compatibility of binaries matters little.
...
\begin{keypoint}[Lesson summary]
  ...
  all later operations, and NOPs bloat the code unless it is compacted with
  stop bits.  Itanium failed in general-purpose computing; DSPs are where
  VLIW fits.  Lesson 12 turns from instructions per cycle to memory.
\end{keypoint}
