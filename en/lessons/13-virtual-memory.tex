...
\begin{itemize}
  \item The address space of a single program is much larger than the
    ...
    many orders of magnitude more than any machine has.
...
operating system (\cref{sec:l13-tlb}).  \Cref{fig:l13-mapping} illustrates
...
fault is enormously more expensive than a cache miss.  Page faults must be
...
address is the PFN bits followed by the offset bits.  The VPN has $k$ bits
...
be accessed (read, write, execute).  A PTE is typically about as large as an
...
have entries too.  Every running program needs a page table of this size.
...
regions of their address space and need only a few inner tables.
...
fragmentation grows in proportion to the page size.
...
page size is 4~KB.  Processors also support much larger pages, such as 2~MB
...
access.  With a page table of $L$ levels, every access of the program turns
...
\begin{itemize}
  ...
  \item One entry serves a whole page rather than a single cache block, so a
    few entries cover the memory a program is actively using.
...
previous program are invalidated or tagged with a program identifier.

\begin{figure}[htbp]
  \centering
  \input{figures/l13-tlb-flow}
...
\begin{description}
  ...
  \item[Size] The TLB should hit about as often as the cache, so it must cover
    ...
    as fast as an L1 cache hit, which keeps it from growing larger.
