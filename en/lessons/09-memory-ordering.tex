...
Loads and stores remain.\margin{About 38\% of the instructions a typical
x86 program executes are loads or stores, roughly two loads per store
(Intel, \emph{Smart Memory Access} white paper, 2006).}  They depend on one
another through memory locations in the same way as instructions depend on one
another through registers
...
\end{lstlisting}
The load has a RAW dependence on the store exactly when R4 equals
R2.\caution{Memory has WAR and WAW dependences too, but stores write memory in
program order (\cref{sec:l09-write}), so only RAW needs machinery.}  Two
...
execution of the program, and stores commit in program order.\sidenote{Commit
is when the write becomes irrevocable, not when it reaches the cache: real
cores move the value into a store buffer that drains into the L1 cache
afterwards.  Other cores may see it late, which is what the consistency
models of Lesson 21 constrain.}
...
the store.  The processor must therefore keep, in program order,
the address and value of every store that has not yet committed, and every load
must consult them.\margin{In the pipeline of Lesson 3, the MEM stage performs
...
\begin{enumerate}
  ...
  \item If it reaches the commit pointer without finding a matching store or a
    ...
    reads memory.\caution{Not the forwarding of Lesson 3: there a result is
    bypassed between pipeline stages, here a value is copied from one
    instruction's LSQ entry into another's.}
\end{enumerate}
...
\begin{enumerate}
  ...
  \item The load executes anyway, and a wrong value is detected and corrected
    later.\margin{The pattern of branch prediction (Lesson 4): proceed on a
    guess, check it later, squash what followed if the guess was wrong.}
\end{enumerate}
...
that have overtaken it.\tip{A load looks back for stores, a store looks
forward for loads: each search skips its own kind.}  When I3 computes its
address in cycle~9, it searches the later LSQ entries for loads with the same
address whose C bit is set, that is, loads that have already obtained a
value.
\begin{itemize}
  ...
  \item If the addresses are equal, I5 has obtained a value that I3
    ...
    (\cref{sec:l08-recovery}).\margin{Cancelling costs a pipeline restart ---
    on Skylake-class cores 15--20 cycles, the order of a branch misprediction
    (Agner Fog, microarchitecture manual).}  When I5 executes again, its
    search finds I3, and it receives the value by forwarding.
\end{itemize}
...
policy that high-performance processors use.\sidenote{Cores decide per load
rather than once for all: a predictor says whether this load is likely to
conflict.  The Alpha 21264 makes only those loads wait whose PC appears in
its table of past squashes, cleared periodically so that a load can become
speculative again; store sets \cite{chrysos1998} pair each load with the
stores it has conflicted with.  Intel calls its version memory
disambiguation and has shipped it since 2006.}
...
\begin{description}
  ...
  \item[For a store] the value goes to every later load with the same address,
    ...
    the value of that store instead.\margin{Hardware forwards only a load that
    lies entirely inside one store; on a partial overlap the load waits for
    the store to reach L1 instead, some 12 cycles (Agner Fog).}
\end{description}
...
\end{lstlisting}
  ...
  \texttt{0x48}.\tip{To check a trace, write down the values of a sequential
  execution first: a load that ends with any other value marks a missing
  re-execution.}
\end{example}
...
that other instructions need with their ROB entry.\margin{Real sizes:
Skylake keeps 72 loads and 56 stores in flight against a 224-entry ROB, Ice
Lake 128 and 72 against 352 (Intel optimization manual).}  A load broadcasts
its value on the CDB under the name of its ROB entry, exactly like an
arithmetic result.  A store produces no register value, so nothing is
...
it.\margin{A multicore adds a rule: a load that has already read must also be
squashed if another core writes its address before it commits (Lesson 21).}
