...
A transistor that is off is not a perfect switch, however.  A small current
...
write is thus a read whose latch is changed before it is written back.
...
\Cref{sec:l15-ras-cas,sec:l15-fpm} exploit this difference.
...
(\cref{sec:l15-chip}) whose latch is written back without using the output.
...
reason why DRAM is slow, and its cost grows with the number of rows.
...
only the quick selection of a column.  Accesses to nearby addresses, which
...
attach the memory bus to the processor directly (\cref{fig:l15-connection}b).
