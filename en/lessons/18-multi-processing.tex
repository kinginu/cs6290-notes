...
\lessoninfo{
  ...
  textbook={H\&P \cite{hennessy2017} §1.1--1.2, §1.5, §5.1, ch.~3, App.~B},
...
\begin{itemize}
  ...
  \item \textbf{Faster.}  A higher clock frequency requires a higher supply
    voltage.  Since dynamic power is proportional to $V^{2}f$
...
generation---provided that the software uses all the cores.
...
\begin{marginfigure}
  \resizebox{\linewidth}{!}{\input{figures/l18-scaling}}
...
programs stay below these curves.
...
therefore keeps it to a minimum.
...
With shared memory the whole array is visible to every thread, so nothing
...
\begin{example}[Shared memory]
  \label{ex:l18-sm}
  Every thread executes the following code on the shared array:
...
The lock admits one thread at a time to the update.  The program is also
...
\begin{description}
  ...
  \item[SMT] One core mixes instructions of both threads in the same cycle
    and fills many of the slots that a single thread would leave empty.  The
...
\begin{description}
  ...
  \item[ROB] Each thread can have a separate ROB with its own commit point.
    ...
    instruction.  The small price is that a completed instruction of one
...
only about \SI{10}{\percent} to the cost of a core.
...
App.~B.}  The same virtual address then refers to different data in each
...
\begin{itemize}
  ...
  \item The \term*{TLB} must therefore be thread-aware: each entry records the
    ...
    number to match (\cref{fig:l18-smt-tlb}).
\end{itemize}

\begin{figure}[htbp]
  \centering
  \input{figures/l18-smt-tlb}
...
\begin{example}
  A core has a \SI{64}{KB} L1 data cache.  Threads A, B and C have working
