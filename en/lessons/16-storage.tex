...
\lessoninfo{
  ...
  textbook={H\&P \cite{hennessy2017} §2.2, App.~D},
...
processor and storage is wider still than the memory wall of Lesson 1.  Its
...
\begin{example}
  ...
  \num{2000} = \num{8e8}$ sectors, or $\num{8e8}\times \num{4096} \approx
  \num{3.3e12}$ bytes, i.e.\ about 3.3~TB.
\end{example}
...
main memory.\source{Lesson 16, videos 4--5; H\&P App.~D.}  For a disk whose
...
between zero and one revolution and is half a revolution on average.  Reading $n$ consecutive sectors takes the
fraction $n/(\text{sectors per track})$ of a revolution:
\begin{equation}
...
\begin{description}
  ...
  \item[Seek time] is typically \SIrange{5}{10}{\milli\second} and improves
    only very slowly.  What improvement there is comes mainly from smaller
...
though capacity grew quickly for many years.  Higher recording density does
...
standard: CD, then DVD, then Blu-ray.  A denser recording technology becomes
...
pulled past the read/write head of the drive.\source{Lesson 16, videos 8--9.}  A tape has a large
capacity, and its cartridges are replaceable: one drive can read and write
any number of cheap cartridges.  Its drawback is the way data is reached.
...
pass as well.  It is unsuitable for virtual memory, whose page faults read
...
\begin{example}
  ...
  \SI[per-mode=symbol]{5}{\metre\per\second}.  With
  the tape at its beginning, data in the middle is reached after
  ...
  \SI{21}{\milli\second}, about \num{10000} times faster than the tape
...
disk.\source{Lesson 16, video 10; H\&P §2.2, §D.2.}  Its cost per gigabyte,
...
needs little power, and retains its data without a battery.  Its cost per
...
\begin{example}
  ...
  the I/O bus now account for most of the time of a hit.
\end{example}
...
workloads whose accesses concentrate on a small part of the data.
...
for disks and optical drives, and a USB host controller provides a USB bus,
...
\begin{keypoint}[Lesson summary]
  ...
  quickly for many years.  Optical disks are portable but improve slowly because of
  standardization; magnetic tape offers cheap capacity with sequential access
