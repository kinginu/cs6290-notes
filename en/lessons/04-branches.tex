...
\begin{figure}[htbp]
  \centering
  \input{figures/l04-branch-pipeline}
...
\end{equation}
which for $w=1$ is exactly the CPI of \cref{eq:l04-cpi}.
...
\begin{example}
  ...
  same depth with 3 instructions per stage (36 flushed).
\end{example}
...
Three observations follow.  In the shallow pipeline even predict not taken
...
limits it to a small table---hundreds to a few thousand entries.  The PC is
...
$2^k$ entries is indexed by bits $k+1$ to~2 (\cref{fig:l04-btb}).  With
variable-length instructions (x86) the index starts at bit~0.

\begin{figure}[htbp]
  \centering
  \input{figures/l04-btb}
...
predicts PC+4.  The BTB needs entries only for taken branches, which are far
...
(10) and strong taken (11) (\cref{fig:l04-2bc}).  From a
...
\begin{figure}[htbp]
  \centering
  \input{figures/l04-pshare-gshare}
...
\end{lstlisting}
  ...
  branch looks random, and PShare predicts it poorly.  The global history
...
Neither is better for all branches.  GShare can in principle learn a branch's
...
Both component predictors are trained on every branch as usual.  The
meta-predictor counter changes only when the components disagree in
correctness: it moves towards the component that was right.  When both are
right or both are wrong, it stays unchanged.

\begin{figure}[htbp]
  \centering
  \input{figures/l04-tournament}
...
so those small tables are left for the branches that need them.\margin{Commercial processors have used predictors of this layered
kind, for example Intel's Pentium~M.}

\begin{figure}[htbp]
  \centering
  \input{figures/l04-hierarchical}
...
(\cref{fig:l04-ras}).  Being small, it can fill up when calls nest deeply.
