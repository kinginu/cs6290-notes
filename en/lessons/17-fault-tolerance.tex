...
different measures of dependability.\margin{The model admits no third state:
a service that is slow but still correct, or interrupted for some users only,
has to be counted as one or the other.}

\begin{figure}[htbp]
  \centering
  \input{figures/l17-service-timeline}
...
the MTTF are nearly equal.\caution{Disk and server datasheets print as
\enquote{MTBF} the quantity defined here as MTTF: no repair time is included
in it.}
...
\begin{example}
  ...
  its availability is $1200/1208 \approx \SI{99.34}{\percent}$.\tip{Availability
  is quoted in \enquote{nines}.  Of the \SI{8766}{\hour} in a year,
  \SI{99.9}{\percent} leaves \SI{8.8}{\hour} of interruption,
  \SI{99.99}{\percent} \SI{53}{\minute} and \SI{99.999}{\percent}
  \SI{5}{\minute}.}  A second
...
\begin{description}
  ...
  \item[Transient] A \term{transient fault}[過渡フォールト] occurs once and
    ...
    particle; rewriting the bit restores correct operation.\caution{Most
    memory errors are not of this kind: a field study of Google's servers
    found over \SI{8}{\percent} of DIMMs hit by a correctable error per year,
    dominated by hard faults rather than particle strikes
    \cite{schroeder2009}.}
\end{description}
...
even if it is activated only occasionally.\margin{Five entries of the
\num{2048}-entry lookup table in the Pentium divider held a wrong value;
replacing the chips cost Intel \$475 million.}
...
there.\margin{A processor does this every few instructions: branch
misprediction recovery in Lesson 8 restores a checkpoint of the register map
and re-executes.  Only the granularity differs.}  The re-execution is likely
to succeed if the fault was transient,
and often if it was intermittent; a permanent fault is activated again and
cannot be tolerated this way.\caution{\enquote{Transient} describes the
fault, not the error it leaves behind: the wrong value persists until
something overwrites it.  Re-execution helps only because the checkpoint
predates the error.}  Recovery also takes time.  If restoring the
...
normally, and after a second one failed it was cut
short.\sidenote{Only four of the five identical IBM AP-101 machines voted on
the same program; the fifth ran software written independently, against a
shared design fault.}
...
flipped bits are detected.\tip{Check bits: the smallest $r$ with
$2^r \ge k+r+1$ locates one flip among $k$ data bits, one more detects two.
$k=64$ gives 8, a 72-bit word.}  Disks protect each sector with considerably
...
Several disks can be combined into a set that plays the role of a single
disk, one that can be larger, faster or more reliable than any of its
disks.\source{Lesson 17, video 15; H\&P §D.2; \cite{patterson1988}.}  Such a
set is called a \term{redundant array of independent
disks}[RAID]\index{RAID|see{redundant array of independent disks}} (RAID).
An array has two goals:
\begin{itemize}
...
The array has $N$ times the capacity of one disk.\margin{The unit striped is
a \emph{chunk} of tens to hundreds of kilobytes, not a single sector; each
block drawn in the figures stands for one chunk.}  Requests for blocks on
...
the failure until the new disk holds a complete copy.\margin{$\mathrm{MTTR}_1$
is a copy time, not a service call: a 16~TB disk refilled at 150~MB/s takes
about \SI{30}{\hour}, the figure used in the examples below.}  Data is lost only if
...
\begin{example}
  ...
  \margin{Datasheet MTTFs of \num{1000000}--\num{1500000}~h imply an annual
  failure rate below \SI{1}{\percent}.  Replacement rates measured in the
  field are \SIrange{2}{4}{\percent}, an MTTF nearer \num{300000}~h
  \cite{schroeder2007}.}
...
blocks of its stripe, parity included.\caution{Coding theory calls a loss at
a known position an \emph{erasure}.  One check symbol repairs one erasure,
but locating \emph{and} repairing one error of unknown position needs two.}
If a disk fails, every block on it
...
writes, independently of $N$.\margin{This read--modify--write sequence is
known as the \emph{small-write problem}.  A write that covers a whole stripe
escapes it: the new parity follows from the new data alone, with no read.}

\begin{figure}[htbp]
  \centering
  \input{figures/l17-raid4-write}
...
independent equations per stripe,\margin{$Q$ is normally a Reed--Solomon
check over $\mathrm{GF}(2^8)$, $Q = \sum_i g^i D_i$ for a fixed $g$.  The
differing weights are what make the second equation independent; a second
XOR would only repeat the first.} so any two lost blocks of a stripe can be
...
\begin{itemize}
  ...
  \item \textbf{Writes} must update both check blocks: three reads (old
    ...
    is about $N/6$ times that of one disk; reads remain $N$ times as
    fast.\tip{The write column of \cref{tab:l17-raid} is $N$ divided by the
    accesses one write costs: four for RAID 4 and 5, six for RAID 6.  RAID 4
    is $1/2$ instead because all four hit the one parity disk.}
\end{itemize}
...
one.\margin{Repair also reads every surviving disk end to end.  Consumer
datasheets allow one unrecoverable read error per $10^{14}$ bits; seven 4~TB
disks hold twice that, so a RAID 5 rebuild may itself lose a block.}
The typical case is an operation fault during the repair: the
