...
pieces of hardware.  If the steps are arranged as \term{stages}[ステージ]
...
\begin{description}
  ...
  \item[MEM] \emph{memory}: access data memory for loads and stores.
...
condition and target are known only at the end of EX.  By then the two
...
\begin{figure}[htbp]
  \centering
  \input{figures/pipeline-branch-flush}
  \caption{A taken branch resolved in EX wastes two fetch slots.}
  \label{fig:flush}
\end{figure}

The cost of this on the CPI depends on how often it happens.  Typical
...
\begin{description}
  \item[RAW (read after write)] the later instruction \emph{reads} a value
    ...
    other.
  \item[WAR (write after read)] the later instruction \emph{writes} a
    location that the earlier one \emph{reads}.  An \emph{anti} dependence.
  \item[WAW (write after write)] both instructions \emph{write} the same
    location; the later write must be the one that survives.  An
    \emph{output} dependence.
\end{description}

WAR and WAW are called \term{false dependences}[偽の依存] or \emph{name}
...
\begin{example}
  ...
  I3:  MUL R1, R6, R7     ; WAW on R1 with I1, WAR on R1 with I2
  \end{lstlisting}
\end{example}
...
\begin{enumerate}
  ...
  \item \textbf{Fix} the value by \term{forwarding}[フォワーディング]
    ...
    register file (\cref{fig:forwarding}).
\end{enumerate}

\begin{figure}[htbp]
  \centering
  \input{figures/pipeline-forwarding}
...
forwarded \emph{backwards in time}.  The result of a load is only available
...
\begin{example}
  ...
  ADD R3, R1, R4     ; needs R1 at start of EX (cycle 4): stall 1 cycle
  \end{lstlisting}
  With forwarding but no stall the \texttt{ADD} would use a stale R1;
  without forwarding it would stall two cycles.
\end{example}
...
\begin{itemize}
  \item \textbf{Clock period decreases} (less logic per stage), which
    ...
    the latch overhead per stage does not shrink, so the gain flattens out.
