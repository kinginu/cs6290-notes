...
\begin{marginfigure}
  \resizebox{\linewidth}{!}{\input{figures/l06-same-cycle}}
...
dependence between I2 and I5.\tip{The second word of the name is what the
\emph{earlier} instruction does: in write-after-read the earlier instruction
reads.}  Both write R5, and in program order I5 writes
...
dependences\index{false dependence}: no value
passes between the two instructions.\caution{H\&P calls these \emph{name
dependences}: WAR is an \emph{antidependence}, WAW an \emph{output
dependence}.}  They exist only because the program
...
produced by its closest preceding writer.\sidenote{Renaming can even replace
an instruction: Intel cores since Ivy Bridge execute most register-to-register
moves in the RAT alone, pointing the destination entry at the source's
physical register.  The move then uses no execution unit and has no latency
...
\begin{example}
  ...
  allocated in increasing order.\margin{Real sizes: Skylake has 180 integer
  and 168 vector physical registers for 16 architectural ones of each kind
  (Agner Fog, microarchitecture manual).}  \Cref{tab:l06-rename} lists each renamed instruction
  ...
  as program order requires.\margin{Dependences through memory locations are
  not affected by register renaming; Lesson~9 treats them.}
\end{example}
...
\end{lstlisting}
  ...
  $\text{ILP} = 8/3 \approx 2.67$.\sidenote{Real programs offer little more.
  Wall simulated benchmarks under models ranging from realistic to impossibly
  good---perfect branch prediction, unlimited renaming, perfect alias
  analysis---and found the average parallelism rarely above~7, with about~5
  more common \cite{wall1991}.}  Two pitfalls are visible.  R1 is written
...
video 16; H\&P §3.1.}\caution{H\&P calls this a structural \emph{hazard}: it
comes from the hardware, not from the program, so it is not really a
dependence.}  Branches, however, add control dependences: whether
...
executed and do not count.\margin{Predication (Lesson 5) removes the control
dependence instead, at the price of executing both paths.}
...
\begin{definition}[Instructions per cycle]
  ...
  1/\text{CPI}$.\caution{The IPC of a whole program is total instructions over
  total cycles, not the average of the IPCs of its phases.}
\end{definition}
...
\begin{description}
  \item[Width] It can execute only a limited number $w$ of instructions per
    ...
    four or more.  Whatever the program, $\text{IPC} \le w$.\margin{Real
    widths: Skylake is designed for 4 instructions per clock, Ice Lake
    for~5 (Agner Fog, microarchitecture manual).}
...
\begin{keypoint}[Lesson summary]
  ...
  out-of-order processor.\margin{Both kinds ship in the same phone: Arm's
  Cortex-A55 is 2 wide and in-order, the Cortex-A78 4 wide and out-of-order.}
  The next lesson presents Tomasulo's
