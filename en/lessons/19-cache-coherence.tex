...
machines.  It ends with the additional cache misses that coherence
causes.\caution{Coherence is about one location at a time; the order of
accesses to \emph{different} locations is memory consistency (Lesson 21).}
...
initially holds 5.\margin{X is one word;
caches act on its whole block---64 bytes on x86-64 and most Arm cores, 128
on Apple's M-series (Lesson 12).}
\begin{enumerate}
...
their copies if they hold the block.\caution{Snooping is not a search of
the other caches: each cache watches the bus and looks up only its own
tags.}  In \cref{fig:l19-problem} the write of
...
records whether other caches may hold the block.\margin{One wired-OR wire
carries the shared signal: any cache that holds the block pulls it, so all
caches answer at once.}  When a cache snoops a read of a block that it
holds, it sets its shared bit and signals on a \emph{shared line} of the bus
that a copy exists; the reading cache sets its own shared bit if this signal
is present.  A write to a block whose shared bit
...
protocols in the rest of this lesson are based on it.\margin{Write-update
reached hardware in a few 1980s machines---Xerox PARC's Dragon, DEC SRC's
Firefly---and every processor since invalidates.}
...
block in S is simply discarded.\tip{In MSI every edge follows one of three
rules: a write by this core ends in M, a write by another core in I, a read
in S.}
...
cache stays dirty and memory does not take the block.\margin{A transfer
between caches costs about an L3 hit, some 40 cycles on Skylake; a DRAM
access costs 60--100~ns (Lesson 14).}
...
\begin{description}
  \item[O (owned)] Valid; other caches may hold copies in S.  The block is
    ...
    invalidation and enters M.\caution{\enquote{Owned} is not
    \enquote{exclusive}: other caches keep valid copies in S.  The owner is
    only the cache responsible for the block.}
\end{description}

\begin{figure}[htbp]
  \centering
  \input{figures/l19-mosi}
...
M or O.\margin{The O state is AMD's: the AMD64 manuals specify MOESI, so a
dirty line can pass from core to core without ever reaching memory.}  In
\cref{ex:l19-msi}, MOSI would take core 1 to O instead of S in step 5,
without the memory write, and core 1 would supply the block for core 0's
write in step 6.
...
\cite{papamarcos1984}, the Illinois protocol.}  Programs often read a
variable and then write it, as in an increment, and most variables are used
by only one thread.  For such a block the read misses and enters S, and the write then broadcasts an
...
\begin{description}
  \item[E (exclusive)] Valid and not dirty; no other cache holds a copy.
    ...
    operation.\tip{Two questions fix the state: may another cache hold a
    copy, and is memory stale?  M: no, yes.  E: no, no.  S: yes, no.  O: yes,
    yes.}
\end{description}
...
(\cref{fig:l19-moesi}).  \Cref{tab:l19-states} summarizes the five
states.

\begin{figure}[htbp]
  \centering
  \input{figures/l19-moesi}
...
\begin{example}
  ...
  again.\margin{Intel's multi-socket protocol adds a fifth state, F
  (forward): like O it marks the one sharer that answers reads, but F is
  clean.}  \Cref{tab:l19-protocols} gives the states and operations under the
...
cached.\margin{A bus serves a handful of cores; a server socket today holds
64--128 of them, connected by an on-chip network.}
...
\begin{itemize}
  \item an \emph{exclusive} bit, which indicates that a single cache holds
    the block, in M or E, and can write it without notifying
    anyone;\margin{One presence bit per core per block: for 64 cores and
    64-byte blocks a full directory would take some \SI{13}{\percent} of
    memory (Lesson 22).}
...
\begin{itemize}
  \item \textbf{Read miss.}  If no cache holds the block, memory supplies
    ...
    bit is cleared.  In both cases the reader is added to the
    entry.\sidenote{Directories are older than multicore: Stanford's DASH of
    the early 1990s made a coherent machine of up to 64 processors out of
    commercial workstations plus directory boards, and led to the SGI Origin
    2000.}
...
\begin{example}
  ...
  remain.\tip{Separate what different threads write: \texttt{alignas(64)} on
  x86-64; Linux's \texttt{perf c2c} finds the lines where that is
  missing.}
\end{example}
\begin{marginfigure}
  \resizebox{\linewidth}{!}{\input{figures/l19-false-sharing}}
