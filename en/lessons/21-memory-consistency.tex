...
stores take effect before both reads, neither core does the work.)
...
long before the store commits.  If this happens on both cores
(\cref{fig:l21-reorder}), both loads read 0 and both cores do the work.

\begin{figure}[htbp]
  \centering
  \input{figures/l21-reorder}
...
flag really holds 0, and the stores take effect at commit, in program order.
...
\begin{example}[Interleavings of the two-flag program]
  ...
  core leaves six interleavings (\cref{tab:l21-interleavings}).  In every one
...
\begin{note}[Strong consistency]
  ...
  accesses occur.  As far as the model is concerned, a read may return an old
  value although another core completed a write to the location earlier, as
  long as the read can be placed before that write without contradicting any
  other result.  A system in which the order must also respect real
  time---an access that completes before another begins comes first, so that
  every read returns the value most recently written---provides
  \term{strong consistency}[強い一貫性].
\end{note}
...
completed.  Coherence makes every completed access visible to all cores, so
the order in which the accesses of all cores complete is itself an
interleaving of the kind the definition requires.
...
wait for the bus and the coherence protocol (Lesson 19).  Limited to one
...
\end{itemize}
The core therefore watches the coherence traffic of the other cores for the
...
A replay is needed only when another core writes a location in the few cycles
between the execution and the commit of a load that reads it, which is rare.
Because invalidations refer to whole blocks, a write to another location in
the same block also causes a replay; this costs time but never produces a
wrong result.
...
The orderings that end in a read, RD $\to$ RD in particular, are the natural
candidates for relaxation.  If a load may take effect before the earlier
accesses of its core to other locations, the core of
\cref{sec:l21-implementing} needs neither to watch coherence traffic nor to
replay loads: every load simply executes when it is ready, as in
\cref{fig:l21-reorder} and \cref{tab:l21-ready}.
...
(\cref{fig:l21-msync}).\margin{x86 provides such an instruction,
\texttt{MFENCE}\index{MFENCE}: every load and store that precedes it in
program order completes before the next load or store.}  Between two
\texttt{MSYNC} instructions, accesses may still be reordered.

\begin{figure}[htbp]
  \centering
  \input{figures/l21-msync}
  \caption{\texttt{MSYNC} divides the accesses of a core, in program order,
    into groups.  Accesses within a group may take effect in any order; no
    access takes effect before an access in an earlier group.}
  \label{fig:l21-msync}
\end{figure}
...
that releases it.  A thread that uses a lock (Lesson 20) executes
...
synchronization library rather than in every program.
...
\texttt{unlock} before the other begins with \texttt{lock}.
...
\begin{itemize}
  \item A \textbf{data-race-free program} produces the same results as under
    sequential consistency, whatever consistency model the system provides,
    as long as the system does not reorder the synchronization itself.
    ...
    order could change a value is ordered by synchronization, which the system
    keeps in order.
...
synchronization that the memory system does not recognize: nothing orders the
reader's load of the data after the writer's store; the \texttt{MSYNC}s of
\cref{sec:l21-relaxed} remove the race.  The lost increment of
...
\begin{keypoint}
  A data-race-free program cannot tell relaxed consistency from sequential
  consistency, so relaxing consistency costs a properly synchronized program
  nothing.  A program with data races can observe anything.
\end{keypoint}
...
\begin{description}
  ...
  \item[Causal consistency] Under \term{causal consistency}[因果一貫性] only
    writes that are causally related---the later write was made after reading,
    directly or indirectly, the result of the earlier one---must be seen by
    all cores in the same order.  Unrelated writes may be seen in different
  ...
  \item[Processor consistency]\index{processor consistency} The writes of one
    ...
    different cores may see the writes of different cores in different
    orders.  A core's read may also take effect before its own earlier write
    to another location, so the execution of \cref{fig:l21-reorder} is
    allowed.
\end{description}
...
\begin{description}
  ...
  \item[Release consistency]\index{release consistency} Synchronization is
    ...
    section---the placement of \texttt{MSYNC} in \cref{sec:l21-relaxed}.
  \item[Lazy release consistency]\index{lazy release consistency} Release
    ...
    lock, not at every release.
  \item[Scope consistency]\index{scope consistency} Each lock defines a
    scope.  A thread that enters a critical section sees the updates made in
    earlier critical sections of the same lock, but not necessarily the updates
    made under other locks.
\end{description}
The list is not complete, and new models continue to be proposed.  What they
...
\begin{keypoint}[Lesson summary]
  ...
  synchronization---behaves as under sequential consistency in any such
  model; a program with data races can observe anything.  Weak, release, lazy
  release and scope consistency differ in how they recognize synchronization,
  and causal and processor consistency in which orders of writes all cores
  must agree on.
\end{keypoint}
