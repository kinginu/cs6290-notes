...
Lesson 12 noted that an L1 hit takes one to three clock cycles.  If a lookup
...
\begin{enumerate}
  ...
  \item the data read completes, the data of the matching way is selected,
    ...
    delivered.
\end{enumerate}
A miss is known at the end of the second stage.  Pipelining does not reduce
...
\begin{example}
  ...
  after the access starts, and an access can start in every cycle.
\end{example}
...
included, and it adds to exactly the quantity we are trying to reduce.
...
have 8-way L1 data caches of 32~KB, exactly at this bound.  Real L1 caches are consequently more associative than
their hit time alone would justify, which makes the techniques of the next
...
guess costs the quick check plus the normal lookup, and a
miss is detected only after the normal lookup as well.  If $p$ is the fraction of accesses in which the predicted line hits,
$t_1$ the time to check one line and $t_N$ the lookup time of the whole set,
the average lookup time is
\begin{equation}
...
\begin{definition}[PLRU]
  ...
  keeps one bit per line, initially~0.  An access sets the bit of its line;
...
\begin{keypoint}
  ...
  pointer per set (NMRU) or one bit per line (PLRU); each keeps most of the miss-rate benefit of
  associativity or LRU at a fraction of the hit-time cost.
\end{keypoint}
...
\begin{definition}[Compulsory, capacity and conflict misses]
  ...
  block competing for the same set.
\end{definition}
...
about 256~bytes for a 256~KB cache: a large cache has room for many blocks even when they are
large, so the space wasted on unused data hurts it less.

\begin{figure}[htbp]
  \centering
  \input{figures/l14-block-size}
...
\begin{description}
  \item[Stream buffer] A \term{stream buffer}[ストリームバッファ]
    \cite{jouppi1990} is a sequential prefetcher.  When blocks are accessed in order, it fetches the
    next few blocks into a small buffer beside the cache, from where an
...
\begin{keypoint}
  ...
  helps only if its guesses are both correct and timely.  Loop interchange attacks misses from
  the program side, by making the order of accesses match the layout of the
  data in memory.
\end{keypoint}
...
memory accesses take so long, even 16 to 32 MSHRs continue to help.

\begin{figure}[htbp]
  \centering
  \input{figures/l14-overlap}
...
hit time for a lower miss rate.
...
\begin{definition}[Misses per 1000 instructions]
  ...
  \index{MPKI|see{misses per 1000 instructions}} (MPKI) counts the misses of a cache per 1000 executed instructions.
\end{definition}
...
L2 never chooses a line with this bit set as its victim.
...
\begin{keypoint}[Lesson summary]
  ...
  progress in MSHRs, and by cache hierarchies,
  whose AMAT is computed level by level with local miss rates, while global
  miss rates and MPKI describe how many accesses go past a level.  With LRU
