...
reset.\caution{Elsewhere \enquote{pseudo-LRU} usually names the tree variant:
$N-1$ bits per set steer a binary tree to the victim.  The bit per line here
is a different approximation.}  The bit of the most recently used line is always set, so the
...
conflict misses, but all of these lengthen the hit time.\margin{A fourth C
is added in multiprocessors: coherence misses, caused by another core's write
invalidating the block (Lesson 19).}  The following
